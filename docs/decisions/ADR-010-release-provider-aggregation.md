...
Hai website này không phải tracker BitTorrent. Chúng là công cụ tìm kiếm metadata/magnet, tương tự phần tìm kiếm DHT mà Bitmagnet đã cung cấp. Tại thời điểm ra quyết định:
...
- BTDig trả trang Google reCAPTCHA cho request tìm kiếm tự động.
...
- BT4G chỉ được bật sau PoC custom Cardigann + FlareSolverr đạt yêu cầu. BTDig giữ trạng thái `unavailable` đến khi có API/Torznab ổn định hoặc không còn bắt CAPTCHA đối với luồng server.
...
- BT4G/BTDig chưa được bật chỉ vì có ADR này. Mỗi nguồn cần PoC và fixture test riêng trước khi production enable.
